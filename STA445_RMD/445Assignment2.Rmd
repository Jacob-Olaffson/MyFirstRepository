...
```{r q6}
    ...
    boot.lm <- function(model, N=1000){
      ...
      for( i in 1:N ){
        sample.data <- data %>% sample_frac( replace=TRUE )
        model.boot <- lm( formula, data=sample.data)
...
## Question challenge guy
make functionerino that can make a histogram from a vector

```{r qlala}
ned.flanders <- function(vec.guy) {
  ggplot(data = data.frame(hi.diddly.ho = vec.guy)) +
    geom_histogram(aes(x=hi.diddly.ho))
}

ned.flanders(runif(n=250, min=1, max=30))
```